package com.gopet.pets.infra.mysql;
...
/**
 * Persiste pets direto no MySQL via JDBC (tabela {@code pets}, ver
...
public class MySqlPetStore implements PetStore {
    ...
    private static final String INSERT_SQL = """
            insert into pets
                (id, tutor_id, nome, idade_anos, porte, peso_kg, reativo, agressivo_pessoas, vacinas_em_dia,
                 restricoes_saude, observacoes)
            values
                (:id, :tutorId, :nome, :idadeAnos, :porte, :pesoKg, :reativo, :agressivoPessoas,
                 :vacinasEmDia, :restricoesSaude, :observacoes)
            """;

    private final JdbcClient jdbcClient;

    public MySqlPetStore(@Qualifier("mysqlJdbcClient") JdbcClient jdbcClient) {

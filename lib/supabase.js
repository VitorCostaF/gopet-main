// lib/supabase.js — dados de demonstração + helpers de preço.
// A autenticação (login/sessão) é via Auth0, ver lib/auth.js — não este arquivo. O Supabase
// aqui serve só como banco de dados (Postgres), usado direto pelas rotas server com a service
// role key (ver app/api/reservas, app/api/assinaturas, app/api/webhooks/pagamento) e pelo
// java-api (ver java-api/.../SupabaseReservaStore.java).

// ── Dados de demonstração (usados quando não há Auth0 configurado, ver DEMO_MODE em lib/auth.js) ─
export const DEMO = {
  servicos: [
